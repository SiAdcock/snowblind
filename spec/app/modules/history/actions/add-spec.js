...
describe('Add history action', () => {
  beforeEach(() => {
    mockSetup();
    mockery.registerMock('../../../helpers/fetch', fetchMock);
    addAction = require('../../../../../app/modules/history/actions/add').actionCreator;
  });
  afterEach(() => {
    mockTearDown();
  ...
  describe('isomorphic post', () => {
    it('is called with history add URL', () => {
      addAction();

      expect(fetchMock.post.calledWith('/api/history/add')).to.be.true;
    });
    ...
    it('is called with history point', () => {
